...
class Lagou(object):
    ...
    def get_content_list(self):  # 提取数据
        ...
        for li in li_list:
            ...
            # 发布时间
            item_list.append(
                li.find_elements_by_xpath(".//div[@class='list_item_top']//span[@class='format-time']")[0].text)

            # 公司名
            item_list.append(
                li.find_elements_by_xpath(".//div[@class='list_item_top']//div[@class='company_name']/a")[0].text)
            # 行业
            ...
            # 薪酬
            item_list.append(
                li.find_elements_by_xpath(".//div[@class='list_item_top']//div[@class='p_bot']//span[@class='money']")[
                    0].text)
            # 经验
            item_list.append(
                li.find_elements_by_xpath(".//div[@class='list_item_top']//div[@class='p_bot']//div[@class='li_b_l']")[
                    0].text)
    ...
    def run(self):
        ...
        tim = '%.3f' % float(time.time() - self.startTime)
        print('--' + '耗时' + str(tim) + '秒--')
        logging.info('--' + '耗时' + str(tim) + '秒--')
